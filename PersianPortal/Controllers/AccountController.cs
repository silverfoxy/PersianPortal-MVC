...
namespace PersianPortal.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [Authorize(Roles = "Administrator")]
        public ActionResult RoleSet(UserRoleViewModel userRole)
        {
            try
            {
                ...
                UserManager<IdentityUser> userManager = new UserManager<IdentityUser>(new UserStore<IdentityUser>());
                var dbUser = db.Users.Find(userRole.user.Id);
                if (!dbUser.Roles.Any(r => r.Role.Id == userRole.role.Id))
                {
                    dbUser.Roles.Add(new IdentityUserRole() { RoleId = userRole.role.Id });
                    db.Entry(dbUser).State = System.Data.Entity.EntityState.Modified;
                    db.SaveChanges();
                }
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.SetPasswordSuccess ? " رمز عبور شما با موفقیت تنظیم شد ."
                : message == ManageMessageId.RemoveLoginSuccess ? "The external login was removed."
                : message == ManageMessageId.Error ? "یک خطا رخ داد."
                : "";
